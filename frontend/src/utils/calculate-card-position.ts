...
export function calculateCardPosition({
  ...
  const left
    = markerPosition.x < mapCenter.x
      ? markerPosition.x + OFFSET
      : markerPosition.x - CARD_WIDTH - OFFSET

  // Y 軸：ピン位置が中心より上なら下側、下なら上側にオフセット
  const top
    = markerPosition.y < mapCenter.y
      ? markerPosition.y + OFFSET
      : markerPosition.y - CARD_HEIGHT - OFFSET
