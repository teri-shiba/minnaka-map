...
describe('deleteAccountSchema', () => {
  describe('requiresEmailCheck=true（メールアドレス登録あり）', () => {
    const requiresEmailCheck = true
    const registered = 'test@minnaka-map.com'

    it('メール未入力（プロパティ欠落）は「必須」で失敗する', () => {
      const schema = deleteAccountSchema(requiresEmailCheck, registered)
  ...
  describe('requiresEmailCheck=false（メールアドレス登録なし）', () => {
    const requiresEmailCheck = false

    it('email が無くても成功する（オプショナル）', () => {
      const schema = deleteAccountSchema(requiresEmailCheck, 'ignored@minnaka-map.com')
      const result = schema.safeParse({})
      expect(result.success).toBe(true)
